...
namespace C_Advanced03.Example02
{
    ...
    {
        public static void BobbleSort(T[] arr, SortingAscOrDescDelegate< bool, T,T>? sortingAsceOrDesc)
        {
            //if (arr is not null && arr.Length > 0 && sortingAscOrDesc is not null)
            if(arr?.Length>0 && sortingAsceOrDesc is not null)
            {
                for (int i = 0; i < arr.Length; i++)
                {
                    ...
                    {
                        // Delegate
                        if (sortingAsceOrDesc(arr[j], arr[j + 1]))
                        {
